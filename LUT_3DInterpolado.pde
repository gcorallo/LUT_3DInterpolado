...
void setup() {
  ...
  name="moreBurn";  
  if (insertHeader) {
    colorLines=new String[32*32*32+5];
    for (int i=0;i<5;i++) {
      colorLines[i]="x";
    }
    ind=5;
    ...
    colorLines=new String[32*32*32];
  }  
  for (int k=0;k<32;k++) {
    for (int j=0;j<32;j++) {
      for (int i=0;i<32;i++) {
